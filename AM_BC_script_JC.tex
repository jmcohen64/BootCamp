...
\begin{proof}[Example]
	...
	%Consider any finite set in the continuum $\Z$. Let $z\in \Z$. Let interval $I\subset \Z$ be $(z-1,z+1)$. 
\end{proof}
...
\begin{proof}
	...
	%Now, for all $x\in X$, $i < x < s$. By Lemma 5.10, for any $p\in X$ such that $i < p < s$, there exists $q,t\in X$ such that $i < q < p$ and $p < t < s$. Therefore, $X$ has no first and last point. 

	%Let $A$ be any nonempty, closed, and bounded set. By Theorem 5.16, $\sup X$ and $\inf X$ both exist. Consider the contrapositive of Lemma 3.12, which states that if $\sup X$ is not a limit point, then either $\sup X$ does not exist, or $\sup X \in X$. Since $\sup X$ exists, 
\end{proof}
...
%\begin{lemma}
%	If two intervals have a point $x$ in common, their union is an interval containing $x$.
%\end{lemma}

%\begin{proof}
%	Let $(a,b),(c,d)\subset C$ with $x\in (a,b)$ and $x\in (c,d)$. Thus, $a<x$ and $x<b$. Also, $c<x$ and $x<d$. Let the function $\max(x,y)$ for $x,y\in C$ be defined as
%	\[\max(x,y) = \begin{cases}
%		x \quad &\text{if } x\geq y\\
%		y \quad &\text{if } x<y,\\
%	\end{cases}\]
...
%		y \quad &\text{if } x>y,\\
%	\end{cases}\]
%	The intersection of the intervals $(a,b)$ and $(b,c)$ 
%	
%	
%	By definition 3.10, x is between $\min(a,c)$ and $\max(b,d)$, and by definition 3.13, the set of all possible values for $x$ describe the interval $(\max(a,c),\min(b,d))$.
%\end{proof}
...
\begin{proof}
	...
	%Consider the set of all $C\in \R$ such that $\underline B < C$ and $C < \overline B$. This is the interval $(\underline B,\overline B) \subset \R$. Consider the intersection $(\underline B,\overline B) \cap \tilde{A}$. Since both are intervals containing $A$, by Theorem 3.20, the intersection is an interval containing $A$. Let this interval be $(P_1,P_2)$.  
	
	%Since $(\underline B,\overline B)$ is bounded, and $(a,b)\subset (\underline B,\overline B)$, $(a,b)$ is bounded. Thus, by Lemma 5.21, $\inf (a,b)$ and $\sup (a,b)$ exist. Let $P_1 = \inf (a,b)$ and $P_2 = \sup (a,b)$. 
	%Let $\underline B, \overline B \in \R\setminus \cA$ such that $\underline B < A < \overline B$. Consider the set of all $C\in \R$ such that $\underline B < C$ and $C < \overline B$. This is the interval $(\underline B,\overline B) \subset \R$.  Let $\tilde{A} = (\underline B,\overline B) \cap \cA$.  Since $(\underline B,\overline B)$ is bounded, and $\tilde{A}\subset (\underline B,\overline B)$, $\tilde{A}$ is bounded. Thus, by Lemma 5.21, $\inf \tilde{A}$ and $\sup \tilde{A}$ exist. Let $P_1 = \inf \tilde{A}$ and $P_2 = \sup \tilde{A}$. Consider the interval $(P_1,P_2)$. Then, $A\in (P_1,P_2)$. 
	
	%Since $\tilde{A}\subset \cA$, $\tilde{A} = \{A\in \cA \mid \}$
	
	%Since both $(\underline B,\overline B)$ and $\cA$ are open, $\tilde{A}$ is open by Theorem 4.17.
	
	%Since, $\tilde{A}$ is open, $P_1,P_2\notin \tilde{A}$ by Lemma 5.22. Since $\tilde{A}\subset \cA$, 
	
	%Let $\tilde{A}$ be the set of all $A\in \cA$ such that $\underline B, \overline B \in \R\setminus \cA$ such that $\underline B < A < \overline B$. Thus, $\tilde{A}\subset \cA$. This set is bounded by $\underline B$ and  $\overline B$, so by Lemma 5.21, the infimum and supremum of $\tilde{A}$ exist. Let $P_1 = \inf \tilde{A}$ and $P_2 = \sup \tilde{A}$. By Lemma 5.12, $P_1$ and $P_2$ are limit points of $\tilde{A}$. 
\end{proof}
